...
// which are part of the Wildfire repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="wildfire-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /wildfire-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "wildfirepay-wildfire/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from wildfirepay-wildfire/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'wildfirepay-wildfire/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from wildfirepay-wildfire/develop"
            }
          }

          if (hasCache) {
            sh "cd /wildfire-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /wildfire-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /wildfire-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /wildfire-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /wildfire-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /wildfire-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /wildfire-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
