...
# wildfire_hash
RUN git clone https://github.com/wildfirepay/wildfire_hash
RUN cd wildfire_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} wildfire
RUN useradd -u ${USER_ID} -g wildfire -s /bin/bash -m -d /wildfire wildfire
...
RUN mkdir /wildfire-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /wildfire-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /wildfire-src
