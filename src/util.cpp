...
const char * const BITCOIN_CONF_FILENAME = "wildfire.conf";
const char * const BITCOIN_PID_FILENAME = "wildfired.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "wildfire" is a composite category enabling all Wildfire-related debug output
                if(ptrCategory->count(std::string("wildfire"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Wildfire
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Wildfire
    // Mac: ~/Library/Application Support/Wildfire
    // Unix: ~/.wildfire
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Wildfire";
#else
    // Unix
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty wildfire.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override wildfire.conf
