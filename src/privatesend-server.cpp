...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Wildfire related functionality
...
/*
    ...
    allow endless transaction that would bloat Wildfire and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Wildfire specific functionality
